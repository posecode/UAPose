...
class CommonFunction(BaseFunction):
    ...
    def __init__(self, cfg, **kwargs):
        ...
        self.PE_Name = kwargs.get("PE_Name", "SLDPOSE")
        ##
        self.max_iter_num = 0
    ...
    def train(self, model, epoch, optimizer, dataloader, tb_writer_dict, **kwargs):
        ...
        for iter_step in range(self.max_iter_num):
            input_x, input_sup_A, input_sup_B, target_heatmaps, target_heatmaps_weight, _,_, _, meta = next(self.dataloader_iter)
            ...
            elif self.PE_Name == "SLDPOSE":

                concat_input = torch.cat((input_x, input_sup_A, input_sup_B), 1).cuda()

                outputs = model.forward_train(concat_input , target_heatmaps  , margin = margin  )
    ...
    def eval(self, model, dataloader, tb_writer_dict, **kwargs):
        ...
        with torch.no_grad():
            ...
            for iter_step in range(self.max_iter_num):
                input_x, input_sup_A, input_sup_B, target_heatmaps, target_heatmaps_weight, prev_heatmaps, next_heatmaps, _, meta = next(self.dataloader_iter)
                ...
                if self.PE_Name == 'SLDPOSE':
                    concat_input = torch.cat((input_x, input_sup_A, input_sup_B), 1).cuda()
                    target_supp_cat = torch.cat((prev_heatmaps, next_heatmaps), dim=1).cuda()

                    outputs = model.forward_val(concat_input.cuda(), target_supp_cat , margin = margin  )
                else:
                    outputs = model(input_x.cuda())
